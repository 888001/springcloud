...
import com.hpp.servicehi.constant.Response;
...
public class WXAuth {
    ...
    @RequestMapping("/web")
    @ResponseBody
    public Response webAuth(HttpServletRequest request, @RequestBody Map<String, String> paraMap) throws Exception {
        ...
        response1.put("authUrl", url);
        response.addSuccess(response1);
    ...
    public Map redirect(HttpServletRequest request) {
        ...
        if ("snsapi_userinfo".equals(scope)) {
            ...
            //如果是snsapi_base则获取到access_token的同时，也获取到了openid，snsapi_base式的网页授权流程即到此为止。
            Map userJson = new HashMap<String, String>();
